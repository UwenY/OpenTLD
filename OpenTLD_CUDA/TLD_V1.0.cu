...
//__global__ void GetbbOverlap_gpu(int* dx,int* dy,int* dwidth,int* dheight,float* doverlap, int GridSize)
//{
//
...
//		TempBB[idx][0] = dx[idx];
//		TempBB[idx][1] = dy[idx];
//		TempBB[idx][2] = dwidth[idx];
//		TempBB[idx][3] = dheight[idx];
//		//bool ifoverlap = true;
//
//		if (TempBB[idx][0] > StandardBB[0] + StandardBB[2])   { doverlap[idx] = 0.f; return; }
//		if (TempBB[idx][1] >  StandardBB[1] + StandardBB[3]) { doverlap[idx] = 0.f; return; }
//		if (TempBB[idx][0] + TempBB[idx][2] <  StandardBB[0]) { doverlap[idx] = 0.f; return; }
//		if (TempBB[idx][1] + TempBB[idx][3] <  StandardBB[1]){ doverlap[idx] = 0.f; return; }
//
//		int xOverlap_f = min(TempBB[idx][0] + TempBB[idx][2], StandardBB[0] + StandardBB[2]) - max(TempBB[idx][0], StandardBB[0]);
//		int yOverlap_f = min(TempBB[idx][1] + TempBB[idx][3], StandardBB[1] + StandardBB[3]) - max(TempBB[idx][1], StandardBB[1]);
//
//	    int OverlapArea_f = xOverlap_f*yOverlap_f;
//		int SumArea_f = TempBB[idx][2]*TempBB[idx][3] + StandardBB[2]*StandardBB[3] - OverlapArea_f;
//
//		doverlap[idx] = (float)OverlapArea_f / SumArea_f;
//		
//	}
//
//}
